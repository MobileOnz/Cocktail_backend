 ...
 *
 * <p>분위기 추천은 cocktail_mood.mood_name 을 이 목록으로 IN 검색한다(부분일치가 아니라 완전일치).
 * 그래서 DB 에만 있고 여기 없는 토큰은 조용히 사라진다 — 그 토큰만 가진 칵테일은
 * 여섯 분위기 어디를 눌러도 나오지 않는다. 데이터에 새 분위기 토큰이 생기면
 * 반드시 여기에도 등록할 것.</p>
 *
 * <p>한 토큰은 한 카테고리에만 둔다. 여러 곳에 두면 같은 칵테일이 서로 다른 분위기 추천에
 * 중복으로 뜬다.</p>
 */
